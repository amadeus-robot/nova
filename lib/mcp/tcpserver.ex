  ...
  def dispatch_method("tools/list", params) do
    tools = %{
      "tools" => [
        ...
        "description" => "Creates a new layer with the specified parent layer, becoming the new head layer",
        "inputSchema" => %{
          "type" => "object",
          "properties" => %{
            "parent_layer_id" => %{"type" => "integer", "description" => "ID of the parent layer"}
          },
      ...
      %{
        ...
        "description" => "Stores module-level declarations (use, require, import, alias) for a module",
        "inputSchema" => %{
          "type" => "object",
          "properties" => %{
            ...
            "declarations" => %{
              "type" => "array",
              "description" => "List of declaration lines like ['use GenServer', 'alias Geom.Square as Sq']",
              "items" => "string"
            },
      ...
      %{
        ...
        "description" => "Retrieves module declarations from the specified layer, walking up the hierarchy if needed",
        "inputSchema" => %{
          "type" => "object",
          "properties" => %{
            "module_name" => %{"type" => "string", "description" => "Name of the module"},
      ...
      %{
        ...
        "description" => "Retrieves a function from the specified layer, walking up the hierarchy if needed",
        "inputSchema" => %{
          "type" => "object",
          "properties" => %{
            "module_name" => %{"type" => "string", "description" => "Name of the module"},
      ...
      %{
        ...
        "description" => "Renders a complete module by collecting all functions and declarations for the specified module",
        "inputSchema" => %{
          "type" => "object",
          "properties" => %{
            "module_name" => %{"type" => "string", "description" => "Name of the module to render"},
            "layer_id" => %{"type" => ["integer", "string"], "description" => "Layer ID or :head for current head", "default" => ":head"}
          },
          "required" => ["module_name"]
        }
      ...
      %{
        ...
        "description" => "Deletes a function from the specified layer (creates a tombstone that masks the function in parent layers)",
        "inputSchema" => %{
          "type" => "object",
          "properties" => %{
            "module_name" => %{"type" => "string", "description" => "Name of the module"},
      ...
      %{
        ...
        "description" => "Deletes module declarations from the specified layer (creates a tombstone that masks the declarations in parent layers)",
        "inputSchema" => %{
          "type" => "object",
          "properties" => %{
            "module_name" => %{"type" => "string", "description" => "Name of the module"},
      ...
      %{
        ...
        "description" => "Compacts a layer by removing functions that are identical to their parent layer",
        "inputSchema" => %{
          "type" => "object",
          "properties" => %{
            "layer_id" => %{"type" => ["integer", "string"], "description" => "Layer ID or :head for current head", "default" => ":head"}
      ...
      %{
        ...
        "inputSchema" => %{
          "type" => "object",
          "properties" => %{
            "base_path" => %{"type" => "string", "description" => "Base path for saving layers", "default" => "./layers"}
          },
          "required" => []
        }
      },
      %{
        ...
        "description" => "Loads all layers from disk using ETS file2tab (replaces current state)",
        "inputSchema" => %{
          "type" => "object",
          "properties" => %{
            "base_path" => %{"type" => "string", "description" => "Base path for loading layers", "default" => "./layers"}
          },
          "required" => []
        }
      },
      %{
        ...
        "inputSchema" => %{
          "type" => "object",
          "properties" => %{
            "base_path" => %{"type" => "string", "description" => "Base path to search for layer files", "default" => "./layers"}
          },
          "required" => []
        }
      }
...
def dispatch_method("tools/call", %{"name" => "hfm_store_module_declarations", "arguments" => args}) do
  %{"module_name" => module_name, "declarations" => declarations} = args
  metadata = Map.get(args, "metadata", %{})
  layer_id = Map.get(args, "layer_id", :head)

  case HierarchicalFunctionManager.store_module_declarations(HierarchicalFunctionManager, module_name, declarations, metadata, layer_id) do
    {:ok, identifiers} ->
      {:ok, %{
        "content" => [%{
          "type" => "text",
...
def dispatch_method("tools/call", %{"name" => "hfm_get_module_declarations", "arguments" => args}) do
  %{"module_name" => module_name} = args
  layer_id = Map.get(args, "layer_id", :head)

  case HierarchicalFunctionManager.get_module_declarations(HierarchicalFunctionManager, module_name, layer_id) do
    {:ok, {declarations, metadata, found_layer_id}} ->
      {:ok, %{
        "content" => [%{
          "type" => "text",
          "text" => "Module declarations for #{module_name} (found in layer #{found_layer_id}):\nDeclarations: #{inspect(declarations, pretty: true)}\nMetadata: #{inspect(metadata, pretty: true)}"
        }]
      }}
    {:error, :not_found} ->
      {:ok, %{
...
def dispatch_method("tools/call", %{"name" => "hfm_store_function", "arguments" => args}) do
  ...
  case HierarchicalFunctionManager.store_function(HierarchicalFunctionManager, module_name, function_name, code, metadata, layer_id) do
    {:ok, function_names} ->
      {:ok, %{
        "content" => [%{
          "type" => "text",
          "text" => "Stored function #{module_name}.#{function_name}: #{inspect(function_names)}"
        }]
      }}
    {:error, reason} ->
      {:ok, %{
...
def dispatch_method("tools/call", %{"name" => "hfm_store_functions", "arguments" => args}) do
  ...
  case HierarchicalFunctionManager.store_functions(HierarchicalFunctionManager, functions, layer_id) do
    {:ok, function_names} ->
      {:ok, %{
        "content" => [%{
          "type" => "text",
...
def dispatch_method("tools/call", %{"name" => "hfm_get_function", "arguments" => args}) do
  ...
  case HierarchicalFunctionManager.get_function(HierarchicalFunctionManager, module_name, function_name, layer_id) do
    {:ok, {code, metadata, found_layer_id}} ->
      {:ok, %{
        "content" => [%{
          "type" => "text",
          "text" => "Function #{module_name}.#{function_name} (found in layer #{found_layer_id}):\n#{code}\n\nMetadata: #{inspect(metadata, pretty: true)}"
        }]
      }}
    {:error, :not_found} ->
      {:ok, %{
...
def dispatch_method("tools/call", %{"name" => "hfm_render_module", "arguments" => args}) do
  ...
  case HierarchicalFunctionManager.render_module(HierarchicalFunctionManager, module_name, layer_id) do
    {:ok, module_code} ->
      {:ok, %{
        "content" => [%{
          "type" => "text",
...
def dispatch_method("tools/call", %{"name" => "hfm_list_functions", "arguments" => args}) do
  ...
  {:ok, %{
    "content" => [%{
      "type" => "text",
      "text" => "Functions visible from layer #{layer_id}:\n#{inspect(functions, pretty: true)}"
    }]
  }}
end

def dispatch_method("tools/call", %{"name" => "hfm_list_module_declarations", "arguments" => args}) do
  layer_id = Map.get(args, "layer_id", :head)

  declarations = HierarchicalFunctionManager.list_module_declarations(HierarchicalFunctionManager, layer_id)

  {:ok, %{
    "content" => [%{
      "type" => "text",
      "text" => "Module declarations visible from layer #{layer_id}:\n#{inspect(declarations, pretty: true)}"
    }]
  }}
end
...
def dispatch_method("tools/call", %{"name" => "hfm_delete_function", "arguments" => args}) do
  ...
  case HierarchicalFunctionManager.delete_function(HierarchicalFunctionManager, module_name, function_name, layer_id) do
    :ok ->
      {:ok, %{
        "content" => [%{
          "type" => "text",
...
def dispatch_method("tools/call", %{"name" => "hfm_delete_module_declarations", "arguments" => args}) do
  %{"module_name" => module_name} = args
  layer_id = Map.get(args, "layer_id", :head)

  case HierarchicalFunctionManager.delete_module_declarations(HierarchicalFunctionManager, module_name, layer_id) do
    :ok ->
      {:ok, %{
        "content" => [%{
          "type" => "text",
  ...
  case HierarchicalFunctionManager.compact_layer(layer_id) do
    {:ok, compacted_count} ->
      {:ok, %{
        "content" => [%{
          "type" => "text",
          "text" => "Compacted layer #{layer_id}, removed #{compacted_count} redundant functions"
        }]
      }}
    {:error, reason} ->
      {:ok, %{
...
def dispatch_method("tools/call", %{"name" => "hfm_list_disk_layers", "arguments" => args}) do
  ...
  def test() do
    ...
    {:ok, res} =  dispatch_method("tools/call", %{"name" => "hfm_store_module_declarations", "arguments" => %{"module_name" => module_name, "declarations" => declarations}})
 ...
 code = "def my_code do 1 end"
  {:ok, res} =  dispatch_method("tools/call", %{"name" => "hfm_store_function", "arguments" => %{"module_name" => module_name, "function_name" => function,"code" => code}})
